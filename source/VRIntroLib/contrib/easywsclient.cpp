...
class _DummyWebSocket : public easywsclient::WebSocket
{
  public:
    ...
    void sendBinary(const std::vector<unsigned char>& message) { }
    void sendPing() { }
    void close() { } 
  ...
  public:
    ...
    virtual void _dispatch(Callback_Imp & callable) {
        struct CallbackAdapter : public BytesCallback_Imp
            // Adapt void(const std::string<unsigned char>&) to void(const std::string&)
        {
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            const unsigned char * data = (unsigned char *) &rxbuf[0]; // peek, but don't consume
            ...
            if (ws.mask) {
                ws.masking_key[0] = ((unsigned char) data[i+0]) << 0;
                ws.masking_key[1] = ((unsigned char) data[i+1]) << 0;
                ws.masking_key[2] = ((unsigned char) data[i+2]) << 0;
                ws.masking_key[3] = ((unsigned char) data[i+3]) << 0;
            }
            else {
                ws.masking_key[0] = 0;
                ws.masking_key[1] = 0;
            ...
            if (false) { }
                ...
                if (ws.fin) {
                    callable((const std::vector<unsigned char>) receivedData);
                    receivedData.erase(receivedData.begin(), receivedData.end());
                    std::vector<unsigned char> ().swap(receivedData);// free memory
                }
            }
    ...
    void sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin, Iterator message_end) {
        ...
        // middleware:
        const unsigned char masking_key[4] = { 0x12, 0x34, 0x56, 0x78 };
    ...
    void close() {
        if(readyState == CLOSING || readyState == CLOSED) { return; }
        readyState = CLOSING;
        unsigned char closeFrame[6] = {0x88, 0x80, 0x00, 0x00, 0x00, 0x00}; // last 4 bytes are a masking key
        std::vector<unsigned char> header(closeFrame, closeFrame+6);
        txbuf.insert(txbuf.end(), header.begin(), header.end());
    }
